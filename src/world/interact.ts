...
import { STAGES, stagesDone } from '../gen/shuttle';
...
let nearMap = false, nearClaim: SavedClaim | null = null, nearGate: ReturnType<typeof nearDoor> = null, nearHD: ReturnType<typeof nearHouseDoor> = null, nearDesk = false, nearWk: ReturnType<typeof nearWorks> = null, nearGun: ReturnType<typeof nearTurret> = null;
...
export function updateEntities(dt: number, time: number) {
  ...
  nearGate = nearDoor(); nearGun = nearGate ? null : nearTurret(); nearHD = nearGate || nearGun ? null : nearHouseDoor(); nearDesk = atShuttleDesk(); nearWk = nearGate || nearGun || nearHD || nearDesk ? null : nearWorks();
  ...
  const busy = !!(nearLad || nearMine || nearPow || nearCar || nearWork2 || nearNpc || nearLock || nearChest || nearBoard || nearMap || nearStash || nearVehicle || nearGate || nearHD || nearDesk || nearWk || nearGun || nearCave || caveOut >= 0);
  ...
  else if (nearDesk) { prompt.className = ''; prompt.textContent = `E — the shuttle project (${stagesDone(G.char.shuttle)} of ${STAGES.length} stages done)`; }
  ...
  prompt.style.display = (climbing() || nearLad || nearMine || nearPow || nearCar || nearWork2 || nearGate || nearHD || nearDesk || nearWk || nearGun || nearCave || caveOut >= 0 || nearNpc || nearLock || nearChest || nearBoard || nearMap || nearStash || nearVehicle || nearPortal || nearFood || nearWork || nearClaim || nearGather || nearCook || nearWater) && G.playing && prompt.textContent ? 'block' : 'none';
  const bh = buildHint();
