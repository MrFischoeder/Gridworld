...
export const sunHeight = (t: number) => Math.sin(sunAngle(t));
...
export const daylight = (t: number) => smooth(-0.12, 0.25, sunHeight(t));
/** Dawn / dusk glow, 0..1: strongest while the sun sits on the horizon. */
export const twilight = (t: number) => Math.exp(-(((sunHeight(t) + 0.02) / 0.12) ** 2));
