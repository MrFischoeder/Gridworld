...
import { villageHere } from '../world/overworld';
...
let open = false, here: Poi | undefined;
...
function deliveries(): string {
  ...
  const offers = boardContracts(here).map((o) => `<div class="notice">${describe(o)}<br><span class="rw">deliver ${dueText(o.due)}${o.kind === 'haul' ? ' · the crates are loaded here, at the store and into the trunk of a vehicle by the gates' : ''}</span><br>` +
...
el.root.addEventListener('click', (e) => {
  ...
  if (!b) return;
  if (b.dataset.take) render(accept(b.dataset.take, here));
  else if (b.dataset.claim) render(claim(b.dataset.claim, here));
  else if (b.dataset.drop) render(abandon(b.dataset.drop));
  else if (b.dataset.ctake) { const o = boardContracts(here).find((x) => x.id === b.dataset.ctake); render(o ? acceptOffer(o) : 'That notice is gone.'); }
...
export function openBoard() {
  ...
  here = villageHere(G.pos.x, G.pos.z);
