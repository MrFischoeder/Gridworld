...
describe('village markets', () => {
  ...
  it('every village makes two goods and wants two others; generator villages want fuel; villages differ', () => {
    const seen = new Set<string>();
    for (const v of vs) {
      const s = villageSeed(w, v), p = profileOf(w, v, s);
      expect(p).toEqual(profileOf(w, v, s));
      expect(p.makes.length).toBe(2); expect(p.wants.length).toBe(2);
      for (const g of p.makes) expect(p.wants).not.toContain(g);
      if (powerKind(s) === 'generator') expect(p.wants).toContain('fuel');
